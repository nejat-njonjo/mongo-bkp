<template>
  <div class="home-page">
    <v-list subheader two-line>
      ...
      <v-list-item v-for="database in databases" :key="database.name">
        <v-list-item-avatar>
          <v-icon class="grey lighten-1" dark> mdi-database </v-icon>
        </v-list-item-avatar>

        <v-list-item-content>
          <v-list-item-title v-text="database.name"></v-list-item-title>
          <v-list-item-subtitle
            v-text="'Size: ' + formatBytes(database.sizeOnDisk)"
          ></v-list-item-subtitle>
          <v-list-item-subtitle
            v-text="database.empty ? 'Empty: YES' : 'Empty: NO'"
          ></v-list-item-subtitle>
        </v-list-item-content>
        ...
        <v-list-item-action>
          ...
          <BackupForm
            @close="activeDatabase = null"
            :database="database"
            v-if="activeDatabase === database.name"
          />
        </v-list-item-action>
      </v-list-item>
    </v-list>
