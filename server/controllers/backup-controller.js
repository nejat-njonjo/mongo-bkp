...
function backup(req, res) {
  try {
    ...
    if (directory && database) {
      ...
      createBackup({ host, port, database }, directory)
        .then((onfullfilled) => {
      ...
      createBackup({ host, port, database })
        .then((onfullfilled) => {
...
function createBackup({ host, port, database }, directory = "") {
  return new Promise((resolve, reject) => {
    ...
    const command = `mongodump --db=${database} --out=${
      directory ? directory : defaultOutDir
    }`;
    ...
    const restoreCommand = `mongorestore --uri ${atlasURI} --db ${database} ${
      directory ? directory : defaultOutDir
    }/${database}`;
